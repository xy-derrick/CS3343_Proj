...
public class Tailoring_bat extends EditDecorator {
	protected BufferedImage imgcopy;
	private int x, y, w, h;
	private int xReal, yReal, wReal, hReal;

	public Tailoring_bat(EditCommand wrappee, ArrayList<Object> args) throws ArgsInvalidException {
		super(wrappee);
		...
		if ((this.w >= 100) || (this.w < 0) || (this.w + this.x > 100)) {
			throw new ArgsInvalidException("c must be (0,100], and a+c<=100.Now a = " + this.x
					+ " ,so that the range of c is (0," + (100 - this.x) + "].\n");
		}
		if ((this.h >= 100) || (this.h < 0) || (this.h + this.h > 100)) {
			throw new ArgsInvalidException("d must be (0,100], and b+d<=100.Now b = " + this.y
					+ " ,so that the range of d is (0," + (100 - this.y) + "].\n");
		}

	}

	public Tailoring_bat(EditCommand wrappee, Integer x, Integer y, Integer w, Integer h) throws ArgsInvalidException {
		super(wrappee);
		this.x = x;
		this.y = y;
		this.w = w;
...
//        ip.setImg(img.getSubimage(xReal,yReal,wReal,hReal));
		ip.setImg(img.getSubimage(x, y, w, h));
