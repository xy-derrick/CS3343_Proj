...
import Code.Command.EditDecorator.*;
...
public class transCommand {
    ...
    public static boolean checkPreviewable(transCommand i) {
        if(previewStr.indexOf(i.cmdType) != -1) return true;
        return false;
    }
        ...
        if(obj instanceof transCommand){
            ...
            if(cmdType.equals(obj1.cmdType)&&parameters.size()==obj1.parameters.size()){
                for(Integer i = 0;i<parameters.size();i++){
                    if(!parameters.get(i).equals(obj1.parameters.get(i))) return false;
                }
                return true;
            }
        return false;
    ...
    public String toString() {
        String str ="" ;
        for(Object i:parameters) str+= (i.toString()+" ");
        return " "+ cmdType+" "+str;
    }
