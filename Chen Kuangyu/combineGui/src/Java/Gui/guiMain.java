 ...
 * export部分
 * 1.类型转换
 * String path为用户输入(eg:C:\Users\Desktop)
 * main_software.setCommand(
                    new bmpTransfer(Software.getInstance().getMain_ip(),path));
    main_software.setCommand(
                    new jpgTransfer(Software.getInstance().getMain_ip(),path));
    main_software.setCommand(
                    new gifTransfer(Software.getInstance().getMain_ip(),path));
    main_software.setCommand(
                    new tiffTransfer(Software.getInstance().getMain_ip(),path));
    main_software.setCommand(
                    new pngTransfer(Software.getInstance().getMain_ip(),path));

    2.存储
    String path为用户输入(eg:C:\Users\Desktop)
    main_software.setCommand(
                            new localSave(Software.getInstance().getMain_ip(),path));

    String ip为用户数入(eg:1)
    main_software.setCommand(
                            new cloudSave(Software.getInstance().getMain_ip(),ip));

    3.压缩
    String path为用户输入(eg:C:\Users\Desktop)
    main_software.setCommand(
                            new imagCompress(Software.getInstance().getMain_ip(),path));
 */
...
import Code.Command.Commands.Common.createCopy;
    ...
    private void init() {
        ...
        btnCombine.addActionListener(evt -> {
            try {
                ...
                main_software.setCommand(new CombineFilter(new EditCommand(Software.getInstance().getMain_ip()),
                        Integer.parseInt(ipnum), degree.floatValue()));
        ...
        btnVintage.addActionListener(evt -> {
            try {
                ...
                main_software.setCommand(
                        new VintageFilter(new EditCommand(Software.getInstance().getMain_ip()), noise.floatValue()));
            } catch (ArgsInvalidException e1) {
                e1.printStackTrace();
            }
            main_software.execute();
        ...
        btnCB.addActionListener(evt -> {
            try {
                ...
                main_software.setCommand(new CreateBorder(new EditCommand(Software.getInstance().getMain_ip()),
                        Integer.parseInt(width), Integer.parseInt(choice)));
            } catch (ArgsInvalidException e1) {
                e1.printStackTrace();
            }
            main_software.execute();
        ...
        btnHC.addActionListener(evt -> {
            try {
                ...
                // String path="aaa";
                main_software.setCommand(new HighContrastFilter(new EditCommand(Software.getInstance().getMain_ip()),
        ...
        btnMosaic.addActionListener(evt -> {
            try {
                ...
                main_software.setCommand(
                        new MosaicFilter(new EditCommand(Software.getInstance().getMain_ip()), Integer.parseInt(size)));
            } catch (ArgsInvalidException e1) {
                e1.printStackTrace();
            }
            main_software.execute();
