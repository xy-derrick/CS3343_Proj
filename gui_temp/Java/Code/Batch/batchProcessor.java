...
public class batchProcessor implements Observable{
    ...
    public BufferedImage preview() throws ArgsInvalidException  {
        ...
        for(transCommand i: cmdBuffer){
            if(transCommand.checkPreviewable(i)) temp.add(i);
        }
            //get Img copy
    ...
    public Boolean checkPreviewable(){
        ...
        for(transCommand i: cmdBuffer){
            if(transCommand.checkPreviewable(i)) countForEditCommand++;
        }
        if(countForEditCommand==0) return false;
        return true;
    }
    
    public ArrayList<BufferedImage> getBatchResult(){
        ArrayList<BufferedImage> imgs = new ArrayList<>(); 
        for (notifyIP ip: batchedImg) imgs.add(ip.getImg());
            return imgs;      
